...
You can use \expr{r.matchedLeft()} and \expr{r.matchedRight()} to get substrings to the left and to the right of the matched substring:
...
\begin{itemize}
    ...
    \item in Flash9, PHP, C\# and Java, the native implementation is used
...
Haxe provides a built-in support for (de-)serializing \emph{JSON}\footnote{http://en.wikipedia.org/wiki/JSON} data via \type{haxe.Json} class.

\subsection{Parsing JSON}

Use \expr{haxe.Json.parse} static method to parse \emph{JSON} data into Haxe value:
\begin{lstlisting}
var s = '{"rating": 5}';
...
var o:{name:String, tags:Array<String>} = haxe.Json.parse(s);
...
Use \expr{haxe.Json.stringify} static method encode Haxe value into a \emph{JSON} string:
\begin{lstlisting}
var o = {rating: 5};
...
\type{haxe.Json} API automatically uses native implementation on platforms where it is available, i.e. \emph{JavaScript}, \emph{Flash} and \emph{PHP} and provides its own implementation for all other platforms.
