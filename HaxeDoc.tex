...
%added the todonotes package instead
...
\todototoc
\listoftodos
\todo{Pull a vocabulary page out of the definitions?}
\clearpage
...
\end{lstlisting}
We will explore type inference in detail later in \Fullref{Type Inference}. For now, it is sufficient to say that the variable \expr{myButton} in the code above is known to be an \emph{class instance} of type \type{MySpecialButton}. A \emph{class instance} is one of seven type groups:

\todo{Is ``2.1 Basic Type'' not a type group?}
\begin{description}
...
\define{Compound Type}{A compound type is a type which has sub-types. This includes any type with \tref{type parameters}{Type Parameters} and the \tref{function}{Function} type.}
...
Alternatively, the \emph{haxe.Int32} and \emph{haxe.Int64} classes can be used to ensure correct overflow behavior regardless of the platform.  \todo{I'm guessing there is a performance hit?  ball park of the cost (extra object references, etc)?}
\\
\\
...
\begin{tabular}{| l | l | l | l | l |}
	...
	Operator & Operation & Argument 1 & Argument 2 & Return \\ \hline
        \expr{\textasciitilde} & negation & Float/Int & N/A & Bool \\
	...
	Operator & Operation & Argument 1 & Argument 2 & Return \\ \hline
	\expr{!} & bitwise negation & \type{Int} & N/A & \type{Int} \\	
...
It is common for programming languages to have a single, clean definition for nullability, but haxe has to find a compromise in this regard. Some target languages allow and, in fact, default to \expr{null} for everything, other do not allow \expr{null} for certain types. This necessitates the distinction of two types of target languages:

\define{Static target}{\expr{null} is not a valid value for basic types. This is true for the \target{Flash 9+}, \target{C++}, \target{Java} and \target{C\#} targets.}
\define{Dynamic target}{Dynamic targets are more lenient with their types and allow null values for basic types. They consist of \target{JavaScript}, \target{PHP}, \target{neko} and \target{Flash 6-8}.}
...
\end{description}
As a consequence, the haxe compiler does not allow assigning \expr{null} to a basic type on static targets. In order to achieve this, the basic type has to be wrapped as \type{Null\textless T\textgreater}.  The \emph{\textless T\textgreater} is a type parameter.  See \Fullref{Type Parameters} for more details. 
...
\emph{Optional Parameters and Nullability:}
\todo{Optional parameters is referred to as ``optional Arguments'' later in the manual.  For consistency sake, do we prefer parameter or argument?}Optional parameters also have to be accounted when considering nullability.
In particular, there must be a distinction between \emph{native} optional parameters which are not nullable and Haxe optional parameters which might be.  The distinction is made by using the question-mark optional parameter:
...
\todo{Later the manual uses ``method'' instead of ``function.''  I think they're generally understood to be interchangeable, but for consistency, I think it should use one or the other throughout}
\end{itemize}
See \Fullref{Class Fields} for more details on the structure of classes.  There is a special type in haxe which is compatible with all classes:

\define[Type]{Class$<$T$>$}{This type is compatible with all class types. At compile-time, \type{Class<T>} can be considered the common base type of all class types. However, this relation is not reflected in generated code.}
...
An interface can be understood as the \emph{signature} of a class because it describes the public fields of a class. Interfaces do not provide implementations, but rather pure structural information:
...
\end{lstlisting}
The \expr{implements} keyword here denotes that \type{Point} has a "is-a" relationship to \type{Printable}, i.e. each instance of \type{Point} is also an instance of \type{Printable}\todo{mention polymorphism?}. While a class may only have one parent class, it may implement multiple interfaces through multiple \expr{implements} keywords.
...
\todo{The comment on line 7 is confusing, maybe it should read ``//Red is type enumValue not Enum<Color>''?}
...
\subsection{Anonymous Structure}z
...
\begin{enumerate}
	...
	\item A \emph{colon} separates the key (which must be a valid haxe \todo{What is a valid field-name?} field-name), from the value
	\item\label{valueanytype} The value can be any valid expression
\end{enumerate}
...
\todo{I filled out this part, may need corrected.  -- C}
...
\todo{What is the failure here?  The compiler complains about the call to foo?}
\begin{lstlisting}
...
\todo{I'm gussing it's the non-static targets (JS, php) that make code like this?}
\begin{lstlisting}
...
While Haxe has a static type system, this type system can be bypassed by using the \type{Dynamic} type. A \emph{dynamic value} can be assigned to anything, and anything can be assigned to it. This has several drawbacks:
...
\begin{itemize}
	...
	\item Some common errors (e.g. a typo in a field access) can not be caught at compile-time and.
        \item \Fullref{Dead Code Elimination} cannot detect if a \type{Dynamic} fields is used..
\end{itemize}
...
d.foo;
\end{lstlisting}
Compiling and running this program for the \target{Flash} target yields an error \texttt{Property foo not found on Number and there is no default value}. Without \type{Dynamic}, this would have been detected at compile-time.

\trivia{Dynamic Inference before haxe 3}{The haxe 3 compiler never infers a type to \type{Dynamic}, so users must be explicit about it. Previous haxe versions used to infer arrays of mixed types (e.g. \expr{[1, true, "foo"]}) as \type{Array<Dynamic>}. We found that this behavior introduced problems and thus removed it for haxe 3.}\todo{So what is the type of that array, or is it a compile error now?}

Use of \type{Dynamic} should be minimized.  There are usually better options, but sometimes it is practical. Parts of the haxe \Fullref{Reflection} API use it, and it is sometimes the best option when dealing with custom data structures that are not known at compile-time.

\type{Dynamic} behaves in a special way when being \tref{unified}{Unification} with a \tref{monomorph}{Monomorph}, which is described in \Fullref{Monomorphs}.
...
\subsection{Abstract}
\label{Abstract}
\todo{Different from abstract classes in other languages.  Maybe we should mention that explicitly.}
...
\todo{Change this to a flow chart?}
\begin{enumerate}
...
\todo{Shoudl array access be part of the operator overloading? Are brackets considered an operator?}
...
\todo{The formatting of the pdf is wrong here.  Possibly due to how lessthan and greaterthen are set in the .tex file?}
...
We learned about the different kinds of types in \Fullref{Types} and it is now time to see how they interact. We start off easy by introducing \tref{typedef}{Typedef}, a mechanism to give a name (or alias) to a more complex type. Among other things, this will come in handy when working with types having \tref{type parameters}{Type Parameters}.

A lot of type-safety is achieved by checking if two given types are compatible. That is, the compiler tries to perform \emph{unification} between them, as detailed in \Fullref{Unification}.
...
\end{lstlisting}
A typedef is not a textual replacement, but rather a real type. It can even have \tref{type parameters}{Type Parameters} as the \type{Iterable} type from the standard library demonstrates:
...
We have seen another interesting use of top-down inference when \tref{construction of generic type parameters}{Construction of generic type parameters} was introduced:
...
\todo{Could be more explicit about modules, packages, paths and impact on the file structure.  On that same vein, should probably talk about ``static public function main'' somewhere, probably earlier in the manual?}
...
\end{enumerate}
If the module and type name are equal, the duplicate can be removed, leading to the \expr{haxe.ds.StringMap<Int>}\todo{In the pdf, the expr fuction doesn't handle line wraps well} short version. However, knowing about the extended version helps with understanding how \tref{module sub-types}{Module Sub-Types} are addressed.
...
Special care has to be taken with field names or local variable names that conflict with a package name: Since they take priority over packages, a local variable named \expr{haxe} blocks the entire \expr{haxe} package.\todo{I hope there is there a compiler warning about conflicts like this?}
...
\todo{Make this into a flow chart.}
\begin{enumerate}
...
\label{Class Fields}
\todo{A little different than the 'standard' OOP definitions. A thing contained in an object is usually called a 'member.'  Might be worth pointing out the difference to other 'OOP' languages.}  
...
\todo{I think it makes sense to talk about flags here (static, dynamic, override, public, and private)}
...
The presence of properties has several consequences on the type system. Most importantly, properties are a compile-time feature and thus \emph{require the types to be known}. If we were to assign a class with properties of \todo{Confused by this.  Are they saying the fields are of type dynamic, or the access identifier is dynamic?} \type{Dynamic}, field access would \emph{not} respect accessor methods. Likewise, access restrictions no longer apply and all access is public.
...
The \expr{dynamic} access modifier works exactly like \expr{get} or \expr{set}, but does not check for the existence of the getter or setter methods.  \todo{Does anyone have a good example of when you'd want to do this?}


\subsubsection{Rules for getter and setter}

The visibility of an accessor method has no effect on the accessibility of its property. That is, if a property is \expr{public} and defined to have a getter, definig the getter as \expr{private} will have no effect on accessing the property. \todo{Does the compiler throw a warning when protection levels get crossed like this?  I would almost want this to be a compile error I think...}
...
\todo{I don't understand.  If it isn't a 'physical field' then what is it?  Why would you ever want a field that wasn't a physical field?}
...
\todo{I wrote this part, so it might need some tweaking. --C}
...
\todo{Include some sample code here?}
...
Some other popular object oriented programming languages allow \emph{Method Overloading}.  However, this paradigm does not play well with type inference or dynamic typing, and is therefore not permitted in Haxe.
...
\label{Visibility}
\todo{Is vocabulary consistent: visibility vs. accessibility? If so, what's the difference?}
...
\todo{which targets suffer the most from function calls? Guessing flash?}
...
\todo{So how do we tell if a function performs better inline?  Use a code profiler?}
...
\todo{What about overriding variables?  Something I can easily check when I have the time...}
Overriding fields is instrumental for creating class hierarchies. Many design patterns utilize overrides, but here we will explore only the basic functionality. In order to use overrides a class must inherit from a \tref{parent class}{Inheritance}. Consider the following example:
\todo{change the source to use ``myMethod'', or even a different name, since function and method are almost interchangeable.}
...
\section{Expressions}
\label{Expressions}
\todo{I feel like a quick intro to expressions should come sooner in the manual.}
...
Furthermore, the internal syntax structure treats \tref{identifiers}{def:Identifier} as constants, which may be relevant when working with \tref{macros}{Macros}.


\subsection{Binary Operators}
\todo{talked about these with a table in the ``Basic Types'' section.  Reproduce that conversation here?}
...
\todo{Anonymous Objects are synonymous with Anonymous Structures?  Objects are the same as structures? Want to be sure to use consistent vocab.}
...
\label{Closure}
\label{function}
\todo{I wrote this section, may need adjusted --C}
...
\subsection{switch}
\todo{I pulled a lot of this from the Haxe 2 manual, but I see that it has changed in Haxe 3. So it basically needs reworked, and needs to  better integrate with the Pattern Matching section later on.}
\label{switch}
...
On some platforms, switches on constant values (especially integers) might be optomized for speed \todo{Still true in Haxe 3?  If so, which platforms?}
...
\label{try/catch}
\todo{Talk about the try catch keyword here, but I feel like exceptions should have their own, larger'ish section.}
...
\todo{Is this section going to overlap with the API documentation.  Would it make sense to just pull in that information?}
...
\todo{yeah, not sure how Haxe uses/handles utf8}
%TODO: utf8 crap %
...
Following the rules previously described in \Fullref{Resolution Order}, multiple \expr{using} expressions are checked from bottom to top, with the types within each module as well as the fields within each type being checked from top to bottom. Using a module (as opposed to a specific type of a module, see \Fullref{Modules and Paths}) as static extension brings all its types into context.
...
\todo{There are more static extensions, like DateTools.  Should this list all of the static extensions, or just some exapmles?}
\begin{description}
  \item[\type{StringTools}:] Provides extended functionality on \tref{strings}{String}, such as replacing or trimming.
  \item[\type{Lambda}:] Provides functional methods on iterables\todo{There should probably be a whole section on Iterables?}.
  \item[\type{haxe.EnumTools}:] Provides type information functionality on \tref{enums}{Enum Instance} and their instances.
  \item[\type{haxe.macro.Tools}:] Provides different extensions for working with macros (see \Fullref{Macro Tools}).\todo{Just a stlye question:  When to use Fullref vs tref?}
\end{description}



\trivia{``using'' using}{Since the \expr{using} keyword was added to the language, it has been common to talk about certain problems with ``using using'' or the effect of ``using using''. This makes for awkward English in many cases, so the author of this manual decided to call the feature by its proper name: Static extension.}
...
\end{lstlisting}
Of course the use of single-quote enclosed strings without any interpolation remains valid, but care has to be taken regarding the \$ character as it triggers interpolation. If an actual dollar-sign should be used in the string, \expr{\$\$} \todo{I think \textbackslash would be more inuitive as an escape sequence than \$ Just curious about the design decision.} can be used.
...
\todo{maybe change this example around so it's easier to follow.}
\haxe{assets/ArrayComprehension2.hx}
...
If access is allowed to an \tref{interface}{Interfaces}, it extends to all classes implementing that interface:
...
\subsection{Remoting}
\todo{This section needs completed}
...
\label{Type Building}
\todo{Some missing sections here}
...
\todo{This section needs filled out}
