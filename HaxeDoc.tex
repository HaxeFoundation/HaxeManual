...
\todo{Eventually change this to a flow chart.}
\begin{enumerate}
...
\todo{Eventually make this into a flow chart.}
\begin{enumerate}
...
The \expr{dynamic} access modifier works exactly like \expr{get} or \expr{set}, but does not check for the existence of the getter or setter methods.  \todo{A good example of when you'd want to do this?}
...
\begin{itemize}
	\item the class \type{Base} which has a method named \expr{myMethod} and a constructor,
	...
	\item the \type{Main} class whose \expr{main} method creates an instance of \expr{Child}, assigns it to a variable \expr{child} of explicit type \type{Base} and calls \expr{myMethod()}.
\end{itemize}

The variable \expr{child} is explicitly typed as \type{Base} to highlight an important difference: At compile-time the type is known to be \type{Base}, but the runtime still finds the correct field \expr{myMethod} on class \type{Child}. This example shows that field access is resolved dynamically at runtime.
...
Furthermore, the internal syntax structure treats \tref{identifiers}{Valid Identifier} as constants, which may be relevant when working with \tref{macros}{Macros}.
...
\label{try/catch}
\todo{The exceptions section needs filled out.}
...
\todo{The Standard Library section needs filled in}
Standard library

\subsection{String}
\label{String}

\todo{Is a string a type or class?}
\define[Type]{String}{A String is a sequence of characters.}
\todo{I'm, not sure how Haxe uses/handles utf8 --C}
%TODO: utf8 crap %
...
\label{Iterables}
\todo{The Iterables section needs structured and filled.}
...
\subsection{Remoting}
\todo{The Remoting section needs filled in}
...
\label{Type Building}
\todo{The Type Building section needs filled in}
...
\todo{Compiler Configuration section needs filled out.}
