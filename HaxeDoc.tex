...
\newcommand{\since}[1]{\emph{Since haxe #1}}
...
However, the compiler assumes that a physical field exists only if at least one of the access identifiers is \expr{default} or \expr{null}. If this is not the case, access to the field from within an accessor method causes a compilation error:
...
\begin{itemize}
	...
	\item otherwise it is equal to \expr{i(matchedValue)}.
\end{itemize}
