...
Haxe provides powerful enumeration (short: enum) types, which are actually an \emph{algebraic data type} (ADT). While they cannot have any \tref{expressions}{expression}, they are very useful for describing data structures:
...
\todo{The wording is a bit broken here, I can only infer what it means.}
...
\begin{flowchart}{type-system-resolution-order-diagram}{Resolution order diagram.}
...
\todo{I would add a "inlining" section in Manual/Language Features, then move the content of  Expression/new/inline constructors there}
