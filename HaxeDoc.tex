...
\todo{Is ``Basic Type'' not a type group?}
\begin{description}
...
\\
\textbf{Overflows:}\\
\todo{Took the ideas from old manual.  Still Haxe 3 relevant?}For performance reasons, the Haxe compiler does not enforce any overflow behavior.  The burden of checking for overflows falls to the target platform.  Here are some platform specific notes on overflow behavior:
...
\begin{description}
 ...
 \item[PHP JS Flash 8:] No native \emph{Int} type.  Overflows will occur if they reach their float limit ($2^{52}$).
\end{description}

Alternatively, the \emph{haxe.Int32} and \emph{haxe.Int64} classes can be used to ensure correct overflow behavior regardless of the platform.  \todo{Is there a performance hit for this?}
\\
\\
...
\begin{tabular}{| l | l | l | l | l |}
	...
	Operator & Operation & Argument 1 & Argument 2 & Return \\ \hline
        \expr{++}& increment & \type{Int} & NA & \type{Int}\\
        & & \type{Float} & NA & \type{Float}\\
        \expr{--} & decrement & \type{Int} & NA & \type{Int}\\
        & & \type{Float} & NA & \type{Float}\\
	...
	Operator & Operation & Argument 1 & Argument 2 & Return \\ \hline
        \expr{\textasciitilde} & negation & Float/Int & NA & Bool \\
	...
	Operator & Operation & Argument 1 & Argument 2 & Return \\ \hline
	\expr{!} & bitwise negation & \type{Int} & NA & \type{Int} \\	
...
It is common for programming languages to have a single, clean definition for nullability, but haxe has to find a compromise in this regard. Som target languages allow and, in fact, default to \expr{null} for everything, other do not allow \expr{null} for certain types. This necessitates the distinction of two types of target languages:
...
\define{Dynamic target}{Dynamic targets are more lenient with their types and allow \textit{null} values for basic types. They consist of \target{JavaScript}, \target{PHP}, \target{neko} and \target{Flash 6-8}.}
...
\end{description}
As a consequence, the haxe compiler does not allow assigning \expr{null} to a basic type on static targets. In order to achieve this, the basic type has to be wrapped as \type{Null$<$T$>$}: \todo{reference to generics?}
...
\textbf{Optional Parameters and Nullability:}
Optional parameters also have to be accounted when considering nullability.
In particular, there must be a distinction between \emph{native} optional parameters which are not nullable and Haxe optional parameters which might be.  The distinction is make by using the question-mark optional parameter:

\begin{lstlisting}
// x is an Int (not nullable)
function foo(x : Int = 0) {\ldots}
// y is Null<Int> (nullable)
function bar( ?y : Int) {\ldots}
// z is also Null<Int>
function opt( ?z : Int = -1) {\ldots}
\end{lstlisting}
\todo{rather confusing, could use a concrete example}
...
Interface fields are not limited to methods, they can be variables and properties \todo{referred to as \textit{variable fields} instead of properties before.  What convention is preferred?} as well.
...
\todo{The comment on line 7 is confusing, maybe wrong?}
...
\todo{What is valid haxe field-name?}
\begin{enumerate}
	\item A structure is enclosed in curly braces \expr{$\left\{\right\}$} 
	\item Has a \emph{comma-separated} list of key-value-pairs
	\item A \emph{colon} separates the key (which must be a valid haxe field-name), from the value
...
Optional fields are \emph{nullable} ({\bfseries \textless Null\textgreater Int} in the example).  Please note that optional fields only work for \emph{anonymouse} structure values (i.e. structures that are not explicitly typed).
...
\todo{Is this right, with defaults and null checking?  which targets make this code, JS?}
\begin{lstlisting}
...
d.foo;
\end{lstlisting}
Compiling and running this program for the \target{Flash} \todo{do you have to indicate Flash is a trademark?} target yields an error \texttt{Property foo not found on Number and there is no default value}. Without \type{Dynamic}, this would have been detected at compile-time.

\trivia{Dynamic Inference before haxe 3}{The haxe 3 compiler never infers a type to \type{Dynamic}, so users must be explicit about it. Previous haxe versions used to infer arrays of mixed types (e.g. \expr{[1, true, "foo"]}) as \type{Array<Dynamic>}. We found that this behavior introduced problems and thus removed it for haxe 3.}

Use of \type{Dynamic} should be minimized as there are better options in many situations, but sometimes it is practical. Parts of the haxe \Fullref{Reflection} API use it, and it is sometimes the best option when dealing with custom data structures that are not known at compile-time.
...
\todo{I don't get the value of this?  Can you have two disjointed types, like String, Int?  If so, maybe include that in this code snippet}
...
\subsection{Abstract}
\label{Abstract}
\todo{slightly confusing b/c very different from abstract classes in other languages}
...
\missingfigure{Make this a flow chart}
\begin{enumerate}
...
\todo{part of operator overloading?}
...
\todo{The formatting is wrong here.  maybe the $<$..$>$ is confusing it?}
...
\end{lstlisting}
\todo{discourage this kind of aliasing.  Can make code hard to understand}
...
\todo{Be more explicit about modules, packages, paths and impact on the file structure.  Maybe this is a Java thing?}
...
\end{enumerate}
If the module and type name are equal, the duplicate can be removed, leading to the \expr{haxe.ds.StringMap<Int>}\todo{expr fuction doesn't handle line wraps well} short version. However, knowing about the extended version helps with understanding how \tref{module sub-types}{Module Sub-Types} are addressed.
...
Special care has to be taken with field names or local variable names that conflict with a package name: Since they take priority over packages, a local variable named \expr{haxe} blocks the entire \expr{haxe} package.\todo{is there a compiler warning about conflicts like this?}
...
\todo{probably better with a flow chart}
\begin{enumerate}
...
\label{Class Fields}
\todo{A little different than the 'standard' OOP definitions. For instance, a thing contained in an object is usually called a 'member.'  Redefining is fine, but it would be easier for people transitioning.`}
...
\todo{I don't understand why this would throw an error.  Why would you ever want a field that wasn't a physical field?}
...
\label{Visibility}
\todo{Is vocabulary consistent: visibility vs. accessibility?}
...
\todo{So how do we tell if a function is more performant inline?  Use a code profiler?}
...
\todo{This only talks about overriding methods.  What about overriding variables?}
...
\todo{change the source to use ``myMethod''}
...
\section{Expressions}
\label{Expressions}
\todo{I feel like this section should come sooner in the manual.}
...
\todo{Anonymous Objects are synonymous with Anonymous Structures?  Objects are the same as structures?}
...
\label{Closure}
\label{function}
\todo{I wrote this section, may need tweaked, ~C}
...
\subsection{switch}
\todo{This is wrong!  Haxe 3 does some pattern matching magic.  Look at the pattern matching page on haxe.org}
\label{switch}
...
On some platforms, switches on constant values (especially integers) might be optomized for speed \todo{which platforms?}
...
\todo{There are more static extensions, like DateTools.  Should this list all of the static extensions, or just the most used for example purposes?}
\begin{description}
  \item[\type{StringTools}:] Provides extended functionality on \tref{strings}{String}, such as replacing or trimming.
  \item[\type{Lambda}:] Provides functional methods on \tref{iterables}{Iterables}\todo[inline]{A section on Iterables?}.
  \item[\type{haxe.EnumTools}:] Provides type information functionality on \tref{enums}{Enum Instance} and their instances.
  \item[\type{haxe.macro.Tools}:] Provides different extensions for working with macros (see \Fullref{Macro Tools}).\todo{Fullref vs tref, when to use which}
\end{description}
...
\todo{Better integrate this with the enum sections}
