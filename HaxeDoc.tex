...
Finally, we will venture to the exciting land of \emph{haxe macros} in \Fullref{Macros} to see how some common tasks can be greatly simplified.
...
\end{lstlisting}
We will explore type inference in detail later in \Fullref{Type Inference}. For now, it is sufficient to say that the variable \expr{myButton} in the above code is known to be an \emph{instance of class} \type{MySpecialButton}. A class instance is one of seven type groups:
...
\haxe{assets/HelloWorld.hx}
The function type will be explored in detail in section \Fullref{Function}, but a quick preview helps here: The type of function \expr{main} in the example above can be considered to be \type{Void->Void} which reads as ``it has no arguments and returns nothing''.
...
There is nothing to worry about when working with \expr{null} on dynamic targets. On the other hand, static ones may require some thought. For starters, basic types are initialized to these values:
...
Semantically, this class represents a point in discrete 2-dimensional space, but this is not important here. Let us describe the structure instead:
...
\end{itemize}
Interfaces, unlike structural sub-typing, describe a \emph{static relation} between classes. A given class is only considered to be compatible to an interface if it explicitly says so:

\begin{lstlisting}
class Point implements Printable { }
\end{lstlisting}
The \expr{implements} keyword here denotes that \type{Point} has an "is-a" relationship to \type{Printable}, i.e. each instance of \type{Point} is also an instance of \type{Printable}. While a class may only have one parent class, it may implement multiple interfaces through multiple \expr{implements} keywords.
...
\define[Type]{Enum}{This type is compatible with all enum types. At compile-time, \type{Enum<T>} can be considered to be the common base type of all enum types. However, this relation is not reflected in the generated code.} 

\subsubsection{Enum Constructor}
\label{Enum Constructor}

Similar to classes and their constructors, enums provide a way of instantiating them by using one of their constructors. In contrast, unlike classes, enums provide multiple constructors which can easily be used through their name:
...
\end{lstlisting}
It is worth noting that the use of anonymous structures does not subvert the typing system. The compiler ensures that only available fields are accessed which means the following program does not compile:
...
The fact that type of \expr{point} is known is courtesy of \tref{type inference}{Type Inference} which thankfully saves us from using explicit types for local variables. However, if \expr{point} were a field, explicit typing would be necessary:
...
\end{lstlisting}
To avoid this kind of redundant type declaration, especially for more complex structures, using a \tref{typedef}{Typedef} is advisable:
...
The function type, along with the \tref{monomorph}{Monomorph}, is a type which is usually well-hidden from Haxe users, yet present everywhere. We can make it surface by using \expr{\$type}, a special Haxe identifier which outputs the type its expression has during compilation :

\haxe{assets/FunctionType.hx}

There is a strong resemblance between the declaration of function \expr{test} and the output of the first \expr{\$type} expression, yet there is also a subtle difference:
...
Function \expr{test} has two optional arguments \expr{i} of type \type{Int} and \expr{s} of \type{String}. This is directly reflected in the function type which is outputted by line 3. 
