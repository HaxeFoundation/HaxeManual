...
\todo{What are the rules for type identifiers?}
...
\todo{Base class hasn't been mentioned before, it's the same as parent class?}
...
\end{itemize}
Interfaces, unlike structural subtyping, describe a \emph{static relation} between classes. A given class is only considered to be compatible to an interface if it explicitly states so:
\todo{What is structural subtyping? Needs more explanation (should be previously described / linked).}
...
\todo{JSON-notation = JavaScript Object Notation-notation, perhaps just use JSON or JSON notation (not sure the hyphen is needed)}
