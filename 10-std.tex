...
The available methods are detailed in the API for \href{Reflect}{http://api.haxe.org//Reflect.html} and \href{Type}{http://api.haxe.org//Type.html}.
...
The problem here is that the compiler never actually ``sees'' the type \type{haxe.Template}, so it does not compile it into the output. Furthermore, even if it were to see the type there could be issues arising from using \tref{dead code elimitation}{cr-dce} eliminating types or fields which are only used via reflection.
