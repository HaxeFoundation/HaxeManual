...
\define{Lambda}{define-lambda}{Lambda is a functional language concept within Haxe that allows you to apply a function to a set or \tref{custom iterators}{lf-iterators}. The Lambda class is a collection of functional methods in order to use functional-style programming with Haxe.}

It is ideally used with \expr{using Lambda} (read \tref{Static Extension}{lf-static-extension}) and then acts as an extension to \type{Iterable} types. 
...
\begin{description}
	\item[\expr{Lambda.array}, \expr{Lambda.list}] Convert Iterable to \type{Array} or \type{List}. It returns a copy if it's already of that type.
	...
	\item[\expr{Lambda.exists}] Determine if any element satisfies a criteria.
	...
	\item[\expr{Lambda.concat}] Merg two Iterables, returning a new List.
...
This example demonstrates the Lamda filter and map on a set of strings:
