...
Haxe remoting is a way to talk between different platforms. With Haxe remoting we can communicate transparently, send data and call methods, between server and client side.

\subsection{Remoting Connection}
\label{std-remoting-connection}

In order to use remoting, there must be a connection established first. There are two kinds of Haxe Remoting connections: 
\begin{description}
...
Haxe remoting can be used to communicate across different platforms. There are some target-specific constructors with different purposes to set up a connection:

\begin{description}
	\item[All targets:]
		\begin{description}
			\item[\expr{HttpAsyncConnection.urlConnect(url:String)}]  
				Returns an asynchronous connection to the given URL that can be a Haxe/Server application. 
		\end{description}
		
	\item[Flash:]
		\begin{description}
			\item[\expr{ExternalConnection.jsConnect(name:String, ctx:Context)}]  
				Allows a connection to the local JavaScript Haxe code. The JS Haxe code must be compiled with the class ExternalConnection included. This is only working with Flash Player 8 and higher.
	...
	\item[Javascript:]
		\begin{description}
			\item[\expr{ExternalConnection.flashConnect(name:String, obj:String, ctx:Context)}]  
				Allows communications a given Flash Object. Some HTML properties must be set. The Haxe Flash content must be loaded and it must include the \expr{haxe.remoting.Connection} class. This will only work with Flash 8 and higher. 
		\end{description}
...
Before we actually communicate between platforms, a remoting context have to be defined. This is a shared API that can be called on the connection at the client code.
...
To make this work for Neko target, setup a Neko Web Server, point the url in the Client to \ic{"http://localhost2000/remoting.n"} and compile the Server using \ic{-main Server -neko remoting.n}.

\paragraph{Error handling}

\begin{itemize}
	\item When an error occurs in a asynchronous call, the error handler is called as seen in this example above.
...
The html-page wrapping the js client must be served from the same domain as where the server is running. The same-origin policy restricts how a document or script loaded from one origin can interact with a resource from another origin. Same-origin Policy is used as a means to prevent some of the Cross-site Request Forgery attacks.
...
There is no warranty of any kind that the arguments types will be respected when a method is called using remoting. 
That means even the arguments of function \expr{foo} are typed to \type{Int}, the client will still be able to use strings while calling the method. 
That can lead to security issues in some cases. Within doubt, check the argument type when the function is called by using the \expr{Std.is} method.
