...
It is important to note that \expr{final} may not have the intended effect with types that are not imutable, such as arrays or objects. Even though the variable cannot have a different object assigned to it, the object itself can still be modified using its methods:
...
An \expr{inline new} call can be used to avoid creating a local class instance, extracting its variables into separate variables instead:

\haxe{assets/InlineConstructor.hx}
