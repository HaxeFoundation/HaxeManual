...
Where a map type is expected, \expr{[]} refers to an empty map.
...
\section{final}
\label{expression-final}

\since{4.0}

The \expr{final} keyword can be used in the following situations:

\begin{itemize}
	\item As modifier to a method, like in Java.
	\item As replacement for \expr{var} when declaring fields.	
	\item \expr{final} class to make it non-extensible
	\item \expr{final} function to make a function non-overridable.
	\item \expr{static final x} that require immediate initialization and cannot be written to.
	\item \expr{final x} that require initialization immediately or in the constructor, and cannot be written to otherwise (note however that it \emph{can} be assigned to multiple times in the constructor).
	\item \expr{final} at expression-level, which is like \expr{var} but cannot be assigned to after the initial initialization.
	\item \expr{final field} on structures. This guarantees read-only. The structure only unifies with classes or other structures where \expr{field} is also \expr{final}.
\end{itemize}

The semantics of the latter are as follows:

\begin{itemize}
	\item \expr{static final} vars have to be initialized immediately.
	\item If a class has non-static `final` vars which are not initialized immediately, it requires a constructor which has to assign to all such fields.
	\item \expr{static inline final} works, but has the same semantics as \expr{static inline var}.
	\item Visibility is orthogonal and not affected.
	\item Property syntax is not supported.
\end{itemize}

\begin{lstlisting}
final a = 42; // declare local a
...
Haxe 4 introduced a shorter syntax for defining local functions without a name, very similar to the function type syntax. The argument list is defined between two parentheses, followed by an arrow \expr{->}, followed directly by the expression:
