...
\trivia{Implementing \type{Dynamic} on non-externs}{Starting with Haxe 4, implementing \type{Dynamic} is only allowed on extern classes. In previous versions any class could implement \type{Dynamic}, allowing arbitrary fields to be read or written. Additionally a special \epxr{resolve} method could be used to resolve read access to a non-existend field. A similar feature is now available as field access \tref{operator overload}{types-abstract-operator-overloading} on abstracts.}
...
\haxe[firstline=107,lastline=113]{assets/PatternMatching.hx}
