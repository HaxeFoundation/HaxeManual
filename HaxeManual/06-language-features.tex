...
See also the \tref{Compiler Flags list}{compiler-usage-flags}.
...
It is also possible to construct a \tref{Map}{std-Map} using array comprehension. 

\begin{lstlisting}
var m = [for (i in 0...10) i => Std.string(i)]; 
\end{lstlisting}

Maps are initialized like arrays, but use the map literal syntax with the \ic{=>} operator. The following code would be equivalent:

\begin{lstlisting}
var i = 0;
var m = new Map<Int, String>();
while(i < 10) m.set(i, Std.string(i));
\end{lstlisting}
