...
Adding child elements to the root can be done using the \expr{createElement} method.
\begin{lstlisting}
...
Use the \expr{Xml.parse} static method to parse \emph{XML} data and obtain a Haxe value from it:
...
Use the \expr{xml.toString()} method to encode a Haxe value into a \emph{XML} string:
\begin{lstlisting}
...
\subsection{Fast Xml access}
\label{std-Xml-Fast}

While the base \type{Xml} API is easy to use, it is sometimes a bit painful to do all the checks of \type{Xml} correctness and can result in verbose code. The \href{http://api.haxe.org/haxe/xml/Fast.html}{haxe.xml.Fast} API is here to help by providing a fast dot-syntax access to the most common \type{Xml} methods.
...
\paragraph{Accessors}
There are different accessors that can be used with the Fast API:
\begin{description}
...
var myNode1Value = fast.node.myNode1.innerData; 
// no problem

var myNode2Value = fast.node.myNode2.innerData; 
// ERROR, an exception is thrown!
\end{lstlisting}

In case the application might get self-closed nodes, check \expr{hasNode.innerData} before and default to an alternative value.

\begin{lstlisting}
var myNode2Value = (fast.node.myNode2.hasNode.innerData) ? fast.node.myNode2.innerData : null; 
\end{lstlisting}
