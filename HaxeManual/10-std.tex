...
All Haxe targets except Neko support Unicode in strings by default. The compiler define \expr{target.unicode} is defined when the current target supports Unicode.
...
Due to the nature of threads accessing a shared memory space with all the Haxe variables and objects, it is possible to run into issues due to deadlocks and race conditions. The standard library provides some core synchronization constructs in the \href{https://api.haxe.org/sys/thread/}{\expr{sys.thread}} package.
