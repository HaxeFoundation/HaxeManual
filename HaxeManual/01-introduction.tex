...
\Fullref{lf} concludes the Haxe language reference by describing some features in detail such as \emph{pattern matching}, \emph{string interpolation} and \emph{dead code elimination}.
...
In the following chapter, \Fullref{std}, we explore important types and concepts from the Haxe Standard Library. We then learn about Haxe's package manager in \Fullref{haxelib}.
...
In a few places, this document has \emph{trivia}-boxes. These include background information such as why certain decisions were made during Haxe's development or how a particular feature has changed as Haxe has developed. This information is generally not essential reading and can be skipped if desired:
...
This can be tested by saving the above code to a file named \ic{Main.hx} and invoking the Haxe Compiler like so: \ic{haxe --main Main --interp}. It generates the following output: \ic{Main.hx:3: Hello world}. There are several things to learn from this:
...
\begin{itemize}
	...
	\item The name of the file containing a Haxe class is the same as the name of the class itself (in this case \type{Main.hx}). 
\end{itemize}
...
Haxe 1 saw several minor releases over the course of two years, adding the \target{Flash AVM2} target along with the \emph{haxelib}-tool in August 2006 and the \target{ActionScript 3} target in March 2007. During this period there was a strong focus on improving stability, which resulted in several minor bug-fix releases.

Haxe 2.0 was released in July 2008 and included the \target{PHP} target, courtesy of \emph{Franco Ponticelli}. A similar effort by \emph{Hugh Sanderson} lead to the addition of the \target{C++} target in July 2009 with the Haxe 2.04 release.
