...
In Haxe the same can be done with \href{https://api.haxe.org/js/Lib.html#debug}{\expr{js.Lib.debug()}} function; this inserts a \ic{debugger} statement that will make a breakpoint if a debugger is available. If no debugging functionality is available, this statement has no effect. 
