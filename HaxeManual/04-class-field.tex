...
Both getter and setter may access their physical field for data storage. The compiler ensures that this kind of field access does not go through the accessor method is made from within the accessor method itself, thus avoiding infinite recursion:
...
Overriding fields are instrumental for creating class hierarchies. Many design patterns utilize it, but here we will explore only the basic functionality. In order to use overrides in a class, it is required that this class has a \tref{parent class}{types-class-inheritance}. Let us consider the following example:
...
The \expr{inline} keyword allows function bodies to be directly inserted in the place of calls to them. This can be a powerful optimization tool but should be used judiciously as not all functions are good candidates for inline behavior. The following example demonstrates the basic usage:
...
Inline is not guaranteed to be done. The compiler might cancel inlining for various reasons or a user could supply the \ic{--no-inline} command line argument to disable inlining. The only exception is if the class is \tref{extern}{lf-externs} or if the class field has the \tref{\expr{extern}}{class-field-extern} access modifier, in which case inline is forced. If it cannot be done, the compiler emits an error.
...
The \expr{extern} keyword causes the compiler to not generate the field in the output. This can be used in combination with the \tref{\expr{inline}}{class-field-inline} keyword to force a field to be inlined (or cause an error if this is not possible). Forcing inline may be desirable in abstracts or extern classes.

\trivia{\expr{:extern} metadata}{Prior to Haxe 4, this access modifier could only be applied to a field using the \expr{:extern} \tref{metadata}{lf-metadata}.}
...
The \expr{final} keyword can be used on class fields with the following effects:

\begin{itemize}
	\item \expr{final function ...} to make a function non-overridable in subclasses.
	\item \expr{final x = ...} to declare a constant that must be initialized immediately or in the constructor and cannot be written to.
	\item \expr{inline final x = ...} is the same but \tref{inlines}{class-field-inline} the value wherever it is used. Only constant values can be assigned.
\end{itemize}

\expr{static final} fields must be initialized immediately by providing an expression. If a class has non-static \expr{final} variables which are not initialized immediately, it requires a constructor which has to assign values to all such fields. \expr{final} does not affect \tref{visibility}{class-field-visibility} and it is not supported on \tref{properties}{class-field-property}.
