...
The Haxe Compiler employs a rich type system which helps to detect type-related errors in a program at compile-time. A type error occurs when an invalid operation is performed on a given type. For example, attempting to divide by a String, trying to access a field on an Integer or calling a function with the wrong number of arguments are all type errors.
...
\end{lstlisting}
These explicit type annotations are not required in Haxe, because the compiler can \emph{infer} the type:

\begin{lstlisting}
var myButton = new MySpecialButton(); // Haxe
\end{lstlisting}
We will explore type inference in detail in the \Fullref{type-system-type-inference} section. For now, it is sufficient to say that the variable \expr{myButton} in the above code is known to be an \emph{instance of class} \type{MySpecialButton}. 

The Haxe type system knows seven type groups:

\begin{description}
 \item[\emph{Class instance}:] an object of a given class or interface
 \item[\emph{Enum instance}:] a Haxe enumeration
 \item[\emph{Structure}:] an anonymous structure, i.e. a collection of named fields
 \item[\emph{Function}:] a compound type of several arguments and one return value
 \item[\emph{Dynamic}:] a wildcard type which is compatible with any other type
...
Basic types are not \tref{classes}{types-class-instance} in Haxe, instead they are implemented as \tref{abstract types}{types-abstract} and are tied to the compiler's internal operator-handling as described in the following sections.
...
While every \type{Int} can be used where a \type{Float} is expected (that is, \type{Int} \emph{is assignable to} or \emph{unifies with} \type{Float}), the reverse is not true: assigning a \type{Float} to an \type{Int} might cause loss of precision and is therefore not implicitly allowed.

\subsection{Overflow}
\label{types-overflow}

For performance reasons, the Haxe Compiler does not enforce any overflow behavior. The burden of checking for numeric overflow falls to the target platform. Here are some platform specific notes on overflow behavior:

\begin{description}
 \item[C++, Java, C\#, Neko, Flash:] 32-bit signed integers with usual overflow practices 
 \item[PHP, JS, Flash 8:] No native \emph{Int} type, loss of precision will occur if a number reaches the float limit (2\textsuperscript{52})
\end{description}

If correct overflow behaviour is required, the \emph{haxe.Int32} and \emph{haxe.Int64} classes can be used although this comes at the cost of additional computations depending upon the platform.
...
\begin{tabular}{| l | l | l | l | l |}
	...
	\emph{Note:} With regard to the \type{Float/Int} return type, if one of the operands is of type \type{Float}, the resulting expression will also be a \type{Float}, otherwise it will be an \type{Int}.
...
Haxe precedence differs from other languages in the following cases:
\begin{itemize}
	\item \expr{\%} has lower precedence than \expr{*} and \expr{/} whereas in C these operators have the same precedence
	\item \expr{|}, \expr{\&} and \expr{\^} have the same precedence in Haxe, so they will be always grouped from left-to-right, whereas in C \expr{\&} has lower precedence than \expr{\^} which itself has lower precedence than \expr{|}
...
Values of type \type{Bool} are a common result of test \emph{conditions} in \tref{\expr{if}}{expression-if} and \tref{\expr{while}}{expression-while} expressions. The following \emph{operators} accept and return \type{Bool} values:
...
Haxe guarantees that compound boolean expressions are evaluated from left to right but only as far as necessary at run-time. For instance, an expression like \expr{A \&\& B} will evaluate \expr{A} first and evaluate \expr{B} only if the evaluation of \expr{A} yielded \expr{true}. Likewise, the expression \expr{A || B} will not evaluate \expr{B} if the evaluation of \expr{A} yielded \expr{true}, because the value of \expr{B} is irrelevant in that case. This is important in cases such as this:
...
\define[Type]{Void}{define-void}{Denotes the absence of a type. It is used to indicate that something (usually a function) has no value.}

\type{Void} is a special case in the type system because it is not actually a type. It is used to express the absence of a type, which applies mostly to function return types.
...
Variables of type \type{Void} are not allowed:
\begin{lstlisting}
// Variables of type Void are not allowed so this will not compile
var x:Void;
\end{lstlisting}


The function type will be explored in detail in the section \Fullref{types-function} but a quick preview is relevant here. The return type of the function \expr{main} in the example above is \type{Void}. Since Haxe does not allow variables of type \type{Void} and \expr{main} has no arguments, the parentheses after \expr{main} are empty. We can see then, that this function takes no arguments and returns nothing.
