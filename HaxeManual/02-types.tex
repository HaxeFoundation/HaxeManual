...
Haxe precedence differs from other languages for the following cases:
\begin{itemize}
...
\define[Type]{Void}{define-void}{Denotes the absence of a type. It is used to express that something (usually a function) has no value.}
...
We have already ``seen'' \type{Void} in the initial ``Hello World'' example:
...
It is common for programming languages to have a single, clean definition for nullability. However, Haxe has to find a compromise in this regard due to the nature of Haxe's target languages: While some of them allow and; in fact, default to \expr{null} for anything, others do not even allow \expr{null} for certain types. This necessitates the distinction between two types of target languages:
...
Classes can be declared with the keyword \expr{final} to prevent them from being extended.

\trivia{\expr{:final} metadata}{Prior to Haxe 4, marking classes as final was possible using the \expr{:final} metadata.}
...
Analogously to classes, interfaces can also be marked with the keyword \expr{final}, preventing them from being extended.
...
Haxe provides powerful enumeration (short: enum) types, which are actually an \emph{algebraic data type} (ADT)\footnote{\url{https://en.wikipedia.org/wiki/Algebraic_data_type}}. While they cannot have any \tref{expressions}{expression}, they are very useful for describing data structures:
...
The fields of a structure may also be declared with \expr{final}, which only allows them to be assigned once. Such a structure will only \tref{unify}{type-system-unification} with other types if the corresponding fields are also \expr{final}.
...
In Haxe 4 it is also possible to declare a structure field as optional in the \emph{class notation} by prefixing its name with \ic{?}:
...
Haxe 4 introduces an alternative notation for an extension, separating each extended structure with a \expr{\&} symbol.

\haxe{assets/Extension3.hx}
...
There is a strong resemblance between the declaration of function \expr{test} and the output of the first \expr{\$type} expression, with one subtle difference: the \emph{function return type} appears at the end after a \expr{->} symbol.

In either notation it is obvious that the function \expr{test} accepts one argument of type \type{Int} and one argument of type \type{String} and returns a value of type \type{Bool}. If a call to this function, such as \expr{test(1, "foo")}, is made within the second \expr{\$type} expression, the Haxe typer checks if \expr{1} can be assigned to \type{Int} and if \expr{"foo"} can be assigned to \type{String}. The type of the call is then equal to the type of the value \expr{test} returns, which is \type{Bool}.

Note that argument names are optional in the function type. If a function type has other function types as argument or return type, parentheses can be used to group them correctly. For example, \type{(Int, ((Int) -> Void)) -> Void} represents a function which has one argument of type \type{Int} and one argument of function type \type{Int -> Void} and a return type \type{Void}.

\haxe{assets/FunctionType2.hx}
...
Before Haxe 4, the function type notation was more similar to other functional programming languages, using \expr{->} in place of all the commas separating the argument types. The \expr{test} function above would be typed as \expr{Int -> String -> Bool} in this notation. \expr{test2} would be typed as \expr{Void -> Bool}.
...
\trivia{New function type notation}{The new function type notation was based on the syntax of \tref{arrow functions}{expression-arrow-function}, which were also introduced in Haxe 4.}
...
Function \expr{test} has two optional arguments: \expr{i} of type \type{Int} and \expr{s} of \type{String}. This is directly reflected in the function type output by line 4. 
...
Default values in Haxe are not part of the type and are not replaced at the call-site (unless the function is \tref{inlined}{class-field-inline}). On some targets the compiler may still pass \expr{null} for omitted argument values and generate code similar to this into the function:
\begin{lstlisting}
...
While Haxe has a static type system, this type of system can, in effect, be turned off by using the \type{Dynamic} type. A \emph{dynamic value} can be assigned to anything and anything can be assigned to it. This has several drawbacks:
...
\begin{itemize}
	...
	\item Some common errors, e.g. a typo infield access, cannot be caught at compile-time and likely cause an error at runtime.
...
It serves one purpose - to hold values of any type but to actually use that values, explicit casting is required. That way the code doesn't suddenly become dynamically typed and we keep all the static typing goodness, like advanced type system features and optimizations.
...
\trivia{Basic Types and abstracts}{Before the advent of abstract types, all basic types were implemented as extern classes or enums. While this nicely taken care of some aspects such as \type{Int} being a ``child class'' of \type{Float}, it caused issues elsewhere. For instance, with \type{Float} being an extern class, it would unify with the empty structure \expr{\{\}}, making it impossible to constrain a type to accept only real objects.}
...
By adding \expr{@:from} to a static function, that function qualifies as implicit cast function from its argument type to the abstract. These functions must return the value of the abstract type. They must also be declared \expr{static}.
...
\begin{itemize}
	...
	\item \expr{@:op([])} on a function with two arguments overloads array write accces, with the first argument being the index and the second one being the written value.
	\item \expr{@:op(a.b)} on a function with one argument overloads field read access.
	\item \expr{@:op(a.b)} on a function with two arguments overloads field write access.
\end{itemize}

\haxe{assets/AbstractAccessOverload.hx}
...
Fields which are defined earlier take priority even if they require a \tref{implicit cast}{types-abstract-implicit-casts}.
...
Since Haxe 4, it is possible to declare enum abstracts without using the \expr{:enum} metadata, instead of using the more natural syntax \expr{enum abstract}. Additionally, if the underlying type is \expr{String} or \expr{Int}, the values for the enum cases can be omitted and are deduced by the compiler:

\begin{itemize}
	\item For \type{Int} abstracts, the deduced values increment the last user-defined value or start at zero if no value was declared yet.
	\item For \type{String} abstracts, the deduced value is the identifier of the enum case.
\end{itemize}

\haxe{assets/AbstractEnum2.hx}
...
Introducing custom core-type abstracts is rarely necessary for user code as it requires the Haxe target to be able to make sense of it. However, there could be interesting use-cases for authors of macros and new Haxe targets.
