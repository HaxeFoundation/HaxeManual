...
\subsection{Getting started with PHP}
\label{target-php-getting-started}

Haxe can be a powerful tool for developing PHP applications. Let's look at our first sample.
This is a very simple example showing the toolchain. 
...
The compiler will output a \emph{bin}-folder, which contains the generated PHP classes that prints a basic HTML-page when you run it.

\paragraph{Run the PHP}

To run the PHP you need a web server, such as \href{http://httpd.apache.org/}{Apache}. If you need an easy-to-install Apache distribution you might want to install \href{https://www.apachefriends.org/download.html}{XAMPP}.
To display the content in a browser, you need either copy the content of the \emph{bin}-folder to the \emph{htdocs}-folder of Apache or change the \ic{DocumentRoot} in \ic{httpd.conf} to your project directory.

The generated PHP-code runs for version 5.1.0 and later.
...
\subsection{PHP target Metatags}
\label{target-php-metatags}

There are no specific metatags for the PHP target. 

For the complete list see \tref{Haxe built-in metatags}{cr-metadata}.
