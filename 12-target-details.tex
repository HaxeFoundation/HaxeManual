...
Haxe can be a powerful tool for developing PHP applications. Let's look at our first example showing the toolchain. 
...
To run the PHP a web server is needed, such as \href{http://httpd.apache.org/}{Apache}. An easy-to-install Apache distribution for local development is \href{https://www.apachefriends.org/download.html}{XAMPP}.
To display the content in a browser, either copy the content of the \emph{bin}-folder to the \emph{htdocs}-folder of Apache or change the \ic{DocumentRoot} in \ic{httpd.conf} to the project directory.
