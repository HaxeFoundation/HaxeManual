...
To expose, apply the \expr{@:expose} metadata on the desired class or static fields.
...
By passing globals (like \ic{window} or \ic{exports}) as parameters to our anonymous function in the Javascript module, it is available which allows to expose the Haxe generated module.
...
The package path of the Haxe class is in the same way fully exposed. To rename the class or define a different package for the exposed class use \expr{@:expose("my.package.MyExternalClass")}
...
Compiling the project using \ic{-D shallow-expose} permits the types or static fiels to be available for the surrounding scope of the Haxe generated closure only.
