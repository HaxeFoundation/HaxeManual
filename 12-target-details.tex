...
To display the output in a browser, you need to create a HTML-document called \ic{index.html} and open it.
...
The \tref{externs mechanism}{lf-externs} assumes that the defined types exist at runtime, but assumes nothing about how and where those types are defined. 
To clarify, in most cases you have to add a \ic{<script>}-tag that links to the external library manually to your HTML-document.
...
Note that (only) in extern classes you can overload functions to accept different types of arguments and return values, using the \expr{@:overload} metatag.
...
Beside externs, \tref{Typedefs}{type-system-typedef} can be a great way to name (or alias) complex Javascript or JSON structures.

The package and class name of the extern should be in the same as defined in the external library. If that doesn't, you can rewrites the path of a class using \expr{@:native}.
...
From Haxe, it is possible to call an exposed functions thanks to the \expr{untyped} keyword. This can be useful in some cases if you don't want to write externs. Anything untyped that is valid syntax will generated as it is.

\begin{lstlisting}
untyped window.trackEvent("page1");  
\end{lstlisting}

Using the magic \expr{__js__} function you can inject pure Javascript code fragments into the output. This code can not be validated, so it accepts invalid and code in the output, which is error-prone.
...
\begin{itemize}
	\item To add a \tref{Haxelib library}{haxelib} you can use \expr{-lib <library-name>}. There are many externs for popular native libraries.
...
\subsection{Using Javascript Metatags}
\label{target-javascript-metatags}

This is the list of Javascript specific metatags. See also the complete list of all \tref{Haxe built-in metatags}{cr-metadata}.
