...
To compile, run this from the command line:
...
To display the output in a browser, create a HTML-document called \ic{index.html} and open it.
...
Note that (only) in extern classes functions can be overloaded to accept different types of arguments and return values, using the \expr{@:overload} metatag.
...
The package and class name of the extern should be in the same as defined in the external library. If that doesn't, rewrite the path of a class using \expr{@:native}.
...
From Haxe, it is possible to call an exposed functions thanks to the \expr{untyped} keyword. This can be useful in some cases if we don't want to write externs. Anything untyped that is valid syntax will generated as it is.

\begin{lstlisting}
untyped window.trackEvent("page1");  
\end{lstlisting}

Using the magic \expr{__js__} function we can inject pure Javascript code fragments into the output. This code can not be validated, so it accepts invalid and code in the output, which is error-prone.
