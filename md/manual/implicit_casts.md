...
Similarly, adding `@:to` to a function qualifies it as implicit cast function from the abstract to its return type. These functions are typically member-functions, but they can be made `static` and then serve as [selective function](selective_functions.md).
...
```
This can be further optimized by [inlining](inline.md) both cast functions, turning the output to this:
...
Previous section: [Abstract](abstract.md)

Next section: [Operator Overloading](operator_overloading.md)