...
Following the rules previously described in [Resolution Order](resolution_order.md), multiple `using` expressions are checked from bottom to top, with the types within each module as well as the fields within each type being checked from top to bottom. Using a module (as opposed to a specific type of a module, see [Modules and Paths](modules_and_paths.md)) as static extension brings all its types into context.

---

Previous section: [Conditional Compilation](conditional_compilation.md)

Next section: [In the Haxe Standard Library](static_extension_in_the_haxe_standard_library.md)