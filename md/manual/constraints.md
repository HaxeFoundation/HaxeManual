...
```
Type parameter `T` of method `test` is constrained to the types `Iterable<String>` and `Measurable`. The latter is defined using a [typedef](typedef.md) for convenience and requires compatible types to have a read-only [property](property.md) named `length` of type `Int`. The constraints then say that a type is compatible if
...
Previous section: [Type Parameters](type_parameters.md)

Next section: [Generic](generic.md)