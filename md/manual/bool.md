...
.
Values of type `Bool` appear commonly in **conditions** such as `if` (section [if](if.md)) and `while` (section [while](while.md)). The following **operators** accept and return `Bool` values:
...
Previous section: [Numeric Operators](numeric_operators.md)

Next section: [Void](void.md)