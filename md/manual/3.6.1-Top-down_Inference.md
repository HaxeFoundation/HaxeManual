...
Here, the compiler knows while typing `[1, "foo"]` that the expected type is `Array<Dynamic>`, so the element type is `Dynamic`. Instead of the usual unification behavior where the compiler would attempt (and fail) to determine a [common base type](3.5.5-Common_Base_Type.md), the individual elements are typed against and unified with `Dynamic`.

We have seen another interesting use of top-down inference when [construction of generic type parameters](3.3.1-Construction_of_generic_type_parameters.md) was introduced:
...
Previous section: [Type Inference](3.6-Type_Inference.md)

Next section: [Limitations](3.6.2-Limitations.md)