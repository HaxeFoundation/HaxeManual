...
If the module and type name are equal, the duplicate can be removed, leading to the `haxe.ds.StringMap<Int>` short version. However, knowing about the extended version helps with understanding how [module sub-types](type-systemmodule-sub-types.md) are addressed.

Paths can be shortened further by using an [import](type-systemimport.md), which typically allows omitting the package part of a path. This may lead to usage of unqualified identifiers, for which understanding the [resolution order](type-systemresolution-order.md) is required.
...
Previous section: [Limitations](type-systeminference-limitations.md)

Next section: [Module Sub-Types](type-systemmodule-sub-types.md)