...
Classes can **implement** ([Interfaces](2.3.3-Interfaces.md)) `Dynamic` and `Dynamic<T>`, which enables arbitrary field access. In the former case, fields can have any type, in the latter they are constrained to be compatible with the parameter type:
...
Previous section: [Dynamic with Type Parameter](2.7.1-Dynamic_with_Type_Parameter.md)

Next section: [Abstract](2.8-Abstract.md)