...
```
The function type will be explored in detail in [Function](2.6-Function.md), but a quick preview helps here: The type of function `main` in above example can be considered to be `Void->Void`, which reads as "it has no arguments and returns nothing".
...
Previous section: [Bool](2.1.2-Bool.md)
