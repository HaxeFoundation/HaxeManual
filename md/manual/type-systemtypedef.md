...
```
A typedef is not a textual replacement, but actually a real type. It can even have [type parameters](type-systemtype-parameters.md) as the `Iterable` type from the standard library demonstrates:
...
Next section: [Extensions](type-systemextensions.md)