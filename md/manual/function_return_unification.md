...
Unification of function return types may involve the [`Void`-type](void.md) and require a clear definition of what unifies with `Void`. With `Void` describing the absence of a type, it is not assignable to any other type, not even `Dynamic`. This means that if a function is explicitly declared as returning `Dynamic`, it must not return `Void`.
...
Previous section: [Monomorphs](monomorphs.md)

Next section: [Common Base Type](common_base_type.md)