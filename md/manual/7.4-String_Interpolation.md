...
```
Of course the use of single-quote enclosed strings without any interpolation remains valid, but care has to be taken regarding the $ character as it triggers interpolation. If an actual dollar-sign should be used in the string, `$$` > I think \\ would be more inuitive as an escape sequence than $ Just curious about the design decision. can be used.
