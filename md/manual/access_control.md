...
Access control can be used if the basic [visibility](visibility.md) options are not sufficient. It is applicable at **class-level** and at **field-level** and knows two directions:



* Allowing access: The target is granted access to the given class or field by using the `:allow(target)` [metadata](metadata.md).
* Forcing access: A target is forced to allow access to the given class or field by using the `:access(target)` [metadata](metadata.md).
...
If access is allowed to an [interface](interfaces.md), it extends to all classes implementing that interface:
...
Previous section: [Metadata](metadata.md)

Next section: [Compiler Features](compiler_features.md)