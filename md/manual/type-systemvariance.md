...
If `Array` had no `push()` method and no other means of modification, the assignment would be safe because no incompatible type could be added to it. We can achieve this in Haxe by restricting the type accordingly using [structural subtyping](type-systemstructural-subtyping.md):
...
Previous section: [Construction of generic type parameters](type-systemgeneric-type-parameter-construction.md)

Next section: [Unification](type-systemunification.md)