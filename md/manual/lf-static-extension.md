...
Following the rules previously described in [Resolution Order](type-systemresolution-order.md), multiple `using` expressions are checked from bottom to top, with the types within each module as well as the fields within each type being checked from top to bottom. Using a module (as opposed to a specific type of a module, see [Modules and Paths](type-systemmodules-and-paths.md)) as static extension brings all its types into context.

---

Previous section: [Conditional Compilation](lf-condition-compliation.md)
