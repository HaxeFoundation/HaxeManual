...
If the value of an `if` expression is required, e.g. for `var x = if(condition) expression1 else expression2`, the typer ensures that the types of `expression1` and `expression2` [unify](3.5-Unification.md). If no `else` expression is given, the type is inferred to be `Void`.

---

Previous section: [while](5.14-while.md)

Next section: [switch](5.16-switch.md)