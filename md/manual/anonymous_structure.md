...
3. A **colon** separates the key, which must be a [Identifier](escapeAnchor(dictionary.md#Identifier)), from the value.
...
The type of `point` is known through [type inference](type_inference.md), which thankfully saves us from using explicit types for local variables. However, if `point` was a field, explicit typing would be necessary:
...
```
To avoid this kind of redundant type declaration, especially for more complex structures, it is advised to use a [typedef](typedef.md):
...
Previous section: [Enum Constructor](enum_constructor.md)

Next section: [JSON-Notation for Structure Values](json-notation_for_structure_values.md)