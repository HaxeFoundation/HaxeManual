...
**Abstract types** are a compile-time feature.  They define a new type on top of an existing type, and at compile-time, the abstract type is replaced with the underlying type.  Abstract types are typically used to modify or augment the behavior of a "concrete" type.  
Here is an example that defines a new **abstract type**:

>TODO: I think changing the name from "Abstract" would make the example more clear.  The file is called MyAbstract.hx
...
* `Abstract` is the name of the abstract and could be anything conforming to the rules for [valid identifiers](5.1-Valid_Identifier.md).
