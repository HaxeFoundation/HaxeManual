...
The type of `point` is known through [type inference](type-systemtype-inference.md), which thankfully saves us from using explicit types for local variables. However, if `point` was a field, explicit typing would be necessary:
...
```
To avoid this kind of redundant type declaration, especially for more complex structures, it is advised to use a [typedef](type-systemtypedef.md):
...
Next section: [JSON-Notation for Structure Values](types-structure-json-notation.md)