...
The DCE-algorithm works well with typed code, but may fail when [dynamic](dynamic.md) or [reflection](reflection.md) is involved. This may require explicit marking of fields or classes as being used by attributing the following metadata:
...
The compiler automatically defines the flag `dce` with a value of either `"std"`, `"no"` or `"full"` depending on the active mode. This can be used in [conditional compilation](conditional_compilation.md).
...
Previous section: [Compiler Features](compiler_features.md)

Next section: [Completion](completion.md)