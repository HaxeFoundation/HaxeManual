...
```
The `implements` keyword here denotes that `Point` has a "is-a" relationship to `Printable`, i.e. each instance of `Point` is also an instance of `Printable`>TODO: mention polymorphism?. While a class may only have one parent class, it may implement multiple interfaces through multiple `implements` keywords.
...
Interface fields are not limited to methods, they can be variables and properties >TODO: referred to as variable fields instead of properties before.  What convention is preferred? as well.
