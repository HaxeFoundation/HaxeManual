...
* `$v{}`: `Dynamic -> Expr` Generates an expression depending on the type of its argument. This is only guaranteed to work for [basic types](basic_types.md) and [enum instances](enum_instance.md).
...
Previous section: [Reification](reification.md)

Next section: [Type Reification](type_reification.md)